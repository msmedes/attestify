...
export function CitationCard({ citation }: CitationCardProps) {
	...
	return (
		<article
			className="scroll-mt-4 border border-[#20211f] bg-[#ffffff] target:shadow-[6px_6px_0_#c14f2f]"
			id={citationElementId(citation.citationHandle)}
		>
			<div className="border-[#20211f] border-b p-3">
				<div className="mb-2 flex items-start justify-between gap-3">
					<code className="break-all bg-[#f7f7f2] px-1 py-0.5 text-[#3b6d65] text-xs">
				...
				<h3 className="font-semibold">{attestation.subject}</h3>
				<p className="text-[#20211f] text-sm">
					{attestation.predicate}: {attestation.value}
				</p>
			</div>

			<div className="grid gap-3 p-3 text-sm">
				<div className="flex items-center gap-2 text-[#3b6d65]">
				...
				<blockquote className="border-[#c14f2f] border-l-4 bg-[#fffaf2] p-3">
					{attestation.anchorText}
				</blockquote>

				<div className="flex items-start gap-2 text-[#6f716d]">
